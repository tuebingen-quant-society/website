  ...
  return (
    <article className="cp">
      <p className="cp__back">
        <a className="link link--arrow" href={localePath(locale, "members")}>
          <span className="arrow arrow--back" aria-hidden="true">
            ←
          </span>
          <span className="link__label">{copy.back}</span>
        </a>
      </p>
      ...
      <section className="cp__section" aria-labelledby="cp-formalities">
        ...
        <h3 className="cp__subtitle">{copy.formalities.rulesTitle}</h3>
        <ul className="cp__rules">
          {copy.formalities.rules.map((rule) => (
            <li key={rule}>{rule}</li>
          ))}
        </ul>
      </section>
